...
import photo1 from '../assets/ElectroMartMobile.png'
import img1 from '../assets/ElectroMartPC.png'
...
export default function Projects() {
  ...
  const projects = useMemo(() => [
    {
      title: "ElectroMart",
      link: "https://electro-mart-shop.vercel.app/",
      bgColor: "#3884d3",
  ...
  return (
    <section id='projects' ref={scenRef} className='relative text-white'
      style={{
        ...
        <div className={`relative w-full flex-1 flex items-center justify-center ${isMobile ? "-mt-4" : ""}`}>
          {
            projects.map((project, idx) => (
              ...
              >
                <AnimatePresence mode='wait'>
                  {activeIndex === idx && (
                    <motion.h3 key={project.title}
                      ...
                      className={`block text-center text-[clamp(2rem,6vw,5rem)] text-white/95 sm:absolute sm:-top-20 sm:left-[35%] lg:left-[-5%] sm:mb-0 italic font-semibold ${isMobile ? "-mt24" : ""}`}
                      style={{
                ...
                >
                  <img src={project.image} alt={project.title} className='w-full h-max object-cover drop-shadow-xl md:drop-shadow-2xl select-none pointer-events-none'
